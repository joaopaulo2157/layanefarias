<?php
require __DIR__.'/config.php';auth();role(['admin']);
$p=db();
$p->exec(<<<'SQL'
...
SQL);
$columns=[
 'products'=>['image_url TEXT','labor_cost REAL DEFAULT 0','packaging_cost REAL DEFAULT 0','loss_pct REAL DEFAULT 0','desired_margin REAL DEFAULT 50','description TEXT'],
 'orders'=>["production_stage TEXT DEFAULT 'Novo'","art_status TEXT DEFAULT 'Não se aplica'","public_token TEXT","customer_approval_status TEXT DEFAULT 'Pendente'","customer_approval_notes TEXT","priority TEXT DEFAULT 'Normal'","production_due_date TEXT"],
 'users'=>['commission_pct REAL DEFAULT 0']
];
foreach($columns as $table=>$defs){foreach($defs as $def){try{$p->exec("ALTER TABLE $table ADD COLUMN $def");}catch(Throwable $e){}}}
foreach([
 ['store_name','Minha Floricultura'],['whatsapp',''],['loyalty_spend_per_point','10'],['loyalty_point_value','0.50'],['card_fee_pct','0'],['tax_pct','0'],['default_loss_pct','5'],['msg_order_received','Olá {cliente}! Recebemos seu pedido {pedido}. Total: {total}. Acompanhe: {portal}'],['msg_art_ready','Olá {cliente}! A arte do pedido {pedido} está pronta para aprovação: {portal}'],['msg_order_ready','Olá {cliente}! Seu pedido {pedido} está pronto.'],['msg_out_for_delivery','Olá {cliente}! Seu pedido {pedido} saiu para entrega.'],['msg_special_date','Olá {cliente}! Está chegando {ocasiao}. Quer que eu separe opções para {presenteado}?']
] as $x)$p->prepare("INSERT OR IGNORE INTO settings(key,value)VALUES(?,?)")->execute($x);
flash('success','Migração para V6 concluída.');go('index.php');
